...
function onload() {
    ...
    hoverOverlay.addEventListener("dragover", event => {
        dragOverlay.classList.remove('hidden');
        event.preventDefault();
    });
    hoverOverlay.addEventListener("dragleave", event => {
        dragOverlay.classList.add('hidden');
    });
    hoverOverlay.addEventListener("drop", function (event) {
        console.log(event);
        event.preventDefault();
        dragOverlay.classList.add('hidden');
        getFilesFromDataTransferItems(event.dataTransfer.items).then(console.log);
    }, false);
}
